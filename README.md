...
- **Briefings**: plataforma de briefing interna, com a estrutura completa do briefing de job (identificação, contexto, objetivo, público, produto/serviço, mensagem/conceito, escopo e entregáveis, canais, referências, histórico, prazos, orçamento, do's & don'ts, KPIs e pontos a apurar), filtro por status/cliente, geração automática da nomenclatura padrão de card/arquivo a partir da sigla do cliente, e uma tela de detalhe que permite avançar o status (rascunho → enviado → em revisão → aprovado/reprovado) registrando uma nota e mantendo o histórico completo de aprovação.
